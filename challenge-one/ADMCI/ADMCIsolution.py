...
# Get Data on ADMCI.
ADMCItrain = pd.read_csv("data/ADMCItrain.csv")
ADMCItest = pd.read_csv("data/ADMCItest.csv")

# Process data. Whatever you do, apply to test data set too.
ADMCItrain = ADMCItrain.drop(columns=["ID"])
ADMCItest = ADMCItest.drop(columns=["ID"])

# If necessary, we can use X-y formula.
y = ADMCItrain["Label"]
X = ADMCItrain.drop(columns=["Label"])

# Preprocess pipeline.
preproc = StandardScaler()
X_train_val = preproc.fit_transform(X)
X_test = preproc.transform(ADMCItest)

train_val_set = pd.DataFrame(X_train_val)
...
models = [ ('BayesianRidge', BayesianRidge()),
           ('MLPClassifier', MLPClassifier(learning_rate='adaptive', learning_rate_init=0.01, activation='logistic')), 
...
search_spaces = [
    {
        ...
        'alpha_1': Real(1e-9, 1e-6, prior='log-uniform'),
        'alpha_2': Real(1e-9, 1e-6, prior='log-uniform'),
        'lambda_1': Real(1e-9, 1e-6, prior='log-uniform'),
        'lambda_2': Real(1e-9, 1e-6, prior='log-uniform')
    },
    {
...
def get_preds_probs(ensembled_model, flat_models, newdata=X_test):
    ...
    pd.DataFrame(final_preds, index=ADMCItest["ID"], columns=["Label"]).to_csv("final_preds_ADMCI.csv")
    pd.DataFrame(final_probs, index=ADMCItest["ID"], columns=["AD", "CTL"]).to_csv("final_probs_ADMCI.csv")
    return final_probs

get_preds_probs(optimized_gb_model, flat_models, newdata=X_test)
